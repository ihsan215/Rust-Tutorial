...
fn print_help(){
    eprintln!("{} - replace a string a new string" , "Find and replace".green());
    eprintln!("Usage: <target string> <raplecement string> <Input file> <Output File>");
}
...
fn parge_args() -> Arguments {
    let args: Vec<String> = env::args().skip(1).collect();
    if args.len() != 4 {
        print_help();
        eprintln!("{} wrong number of arguments give. Expected 4 , got {}" , "Error".red() , args.len());
        std::process::exit(1);
    }
